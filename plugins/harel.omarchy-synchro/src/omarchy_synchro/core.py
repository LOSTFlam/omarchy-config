...
def copy_snapshot_file(source: Path, target: Path, remaining_bytes: int) -> int:
    copied = 0
    try:
        with source.open("rb") as source_stream, target.open("xb") as target_stream:
            while chunk := source_stream.read(64 * 1024):
                copied += len(chunk)
                if copied > MAX_CAPTURE_FILE_BYTES or copied > remaining_bytes:
                    raise SynchroError("snapshot exceeds its per-file or 256 MiB aggregate byte limit")
...
def build_snapshot(repo: Path, home: Path, stage: Path) -> dict:
    ...
    for entry in entries:
        ...
        for candidate, relative in iter_source_files(source, entry.relative):
            if count >= MAX_SNAPSHOT_FILES:
                raise SynchroError(f"snapshot exceeds the {MAX_SNAPSHOT_FILES}-file capture limit")
            target = target_base / Path(relative.as_posix())
            target.parent.mkdir(parents=True, exist_ok=True)
            total_bytes += copy_snapshot_file(candidate, target, MAX_SNAPSHOT_BYTES - total_bytes)
            count += 1
    ...
    summary = {"schema": 1, "deviceId": device_id, "files": count, "bytes": total_bytes}
    ...
    for current, dirs, files in os.walk(root, followlinks=False):
        ...
        for name in sorted(files):
            ...
            if count > MAX_TREE_FILES:
                raise SynchroError(f"tree comparison exceeds the {MAX_TREE_FILES}-file limit")
            yield candidate.relative_to(root)


def tree_changes(source: Path, destination: Path) -> list[str]:
    changes: list[str] = []
    source_files = set(iter_tree_files(source) or [])
    seen_destination: set[Path] = set()

    def add_change(value: str) -> None:
        if len(changes) >= MAX_SNAPSHOT_CHANGES:
            raise SynchroError(f"snapshot diff exceeds the {MAX_SNAPSHOT_CHANGES}-item review limit")
        changes.append(value)

    for rel in iter_tree_files(destination) or []:
        seen_destination.add(rel)
        if rel not in source_files:
            add_change(f"D {rel}")
        elif not filecmp.cmp(source / rel, destination / rel, shallow=False):
            add_change(f"M {rel}")
    for rel in sorted(source_files - seen_destination, key=str):
        add_change(f"A {rel}")
    return changes
...
def snapshot(repo: Path, home: Path, apply: bool) -> tuple[dict, list[str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="omarchy-synchro-") as temp:
        ...
        for name in MANAGED_DIRS:
            scoped_changes = [f"{name}/{line}" for line in tree_changes(stage / name, repo / name)]
            if len(changes) + len(scoped_changes) > MAX_SNAPSHOT_CHANGES:
                raise SynchroError(f"snapshot diff exceeds the {MAX_SNAPSHOT_CHANGES}-item review limit")
            changes.extend(scoped_changes)
        if apply:
            for name in MANAGED_DIRS:
                target = repo / name

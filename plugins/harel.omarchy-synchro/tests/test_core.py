...
class SafetyTests(unittest.TestCase):
    ...
    def test_qml_untrusted_sinks_are_plain_and_streamed(self):
        ...
        self.assertIn('function refresh() { run("status", ["--json", "status"]) }',overlay)
    ...
    def test_snapshot_enforces_aggregate_file_and_byte_limits(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); home=root/"home"; repo=root/"repo"
            home.mkdir(); repo.mkdir(); (repo/".git").mkdir(); (repo/"policy").mkdir()
            (repo/"policy/allowlist.tsv").write_text("portable\t.config/app\n")
            source=home/".config/app"; source.mkdir(parents=True)
            (source/"one.conf").write_text("one"); (source/"two.conf").write_text("two")
            with patch("omarchy_synchro.core.MAX_SNAPSHOT_FILES",1):
                with self.assertRaisesRegex(SynchroError,"file capture limit"): snapshot(repo,home,False)
            with patch("omarchy_synchro.core.MAX_SNAPSHOT_BYTES",5):
                with self.assertRaisesRegex(SynchroError,"aggregate byte limit"): snapshot(repo,home,False)

    def test_snapshot_diff_has_an_aggregate_item_limit(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); home=root/"home"; repo=root/"repo"
            home.mkdir(); subprocess.run(["git","init","-q",str(repo)],check=True)
            (repo/"policy").mkdir(); (repo/"policy/allowlist.tsv").write_text("portable\t.config/app\n")
            source=home/".config/app"; source.mkdir(parents=True)
            (source/"one.conf").write_text("one"); (source/"two.conf").write_text("two")
            with patch("omarchy_synchro.core.MAX_SNAPSHOT_CHANGES",1):
                with self.assertRaisesRegex(SynchroError,"diff exceeds"): snapshot(repo,home,False)

...
Item {
  ...
  property string snapshotPending: "Not checked"
  ...
  function applyOverview(data) {
    if (!data) return
    snapshotPending = String((data.changes || []).length)
              ...
              Item {
                ...
                Column {
                  ...
                  Row {
                    ...
                    Repeater {
                      model: [
                        { label: "Snapshot pending", value: root.snapshotPending, icon: "󰆓" },
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if ((root.action === "status" || root.action === "status-fetch") && !root.actionFailed) {
        ...
        root.message = Number(root.snapshotPending) > 0
          ? root.snapshotPending + " snapshot change" + (Number(root.snapshotPending) === 1 ? "" : "s") + " pending."
          : "Snapshot and repository status refreshed."
      } else if (root.action === "snapshot-preview" && !root.actionFailed) {
        root.applyOverview(data)
        root.message = Number(root.snapshotPending) > 0
          ? root.snapshotPending + " snapshot change" + (Number(root.snapshotPending) === 1 ? "" : "s") + " pending."
          : "Captured snapshot is up to date."

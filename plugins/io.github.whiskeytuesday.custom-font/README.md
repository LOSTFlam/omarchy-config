...
Two paths — pick whichever fits how you use Omarchy.

### As an Omarchy plugin (with a Quickshell overlay)

```bash
omarchy plugin add https://github.com/whiskeytuesday/om-custom-font.git --enable
omarchy restart shell
```

The shell restart is needed once, after install, so the overlay is wired into
the shell's toggle machinery. Skip it and the overlay won't appear even
though the plugin is enabled.

**Opening the overlay.** Two ergonomic paths — pick either or both. The raw
IPC (`omarchy-shell shell toggle io.github.whiskeytuesday.custom-font`) works
but is verbose:

1. **From the Omarchy menu.** Add one line to
   `~/.config/omarchy/extensions/omarchy-menu.jsonc`:

   ```jsonc
   "install.font-file": {"icon":"","label":"Font from file","action":"omarchy-shell shell toggle io.github.whiskeytuesday.custom-font"},
   ```

   Then `Super+Shift+Space` → type "font" → Enter. It lives next to
   `omarchy install font`, where it belongs.

2. **From a terminal (or a keybind).** Symlink the wrapper this repo ships:

   ```bash
   ln -s ~/.config/omarchy/plugins/io.github.whiskeytuesday.custom-font/bin/omarchy-custom-font ~/.local/bin/
   ```

   Now `omarchy-custom-font` opens the overlay. Bind it to a key if you want:

   ```conf
   # ~/.config/hypr/bindings.conf
   bind = SUPER SHIFT, F, exec, omarchy-custom-font
   ```

Once open: pick a `.ttf`/`.otf` from the inline file browser, tick *Set as
shell default*, choose whether GTK follows, hit **Install**. The overlay is
a thin GUI on top of the CLI below.

To remove:

```bash
omarchy plugin remove io.github.whiskeytuesday.custom-font --yes
```

Removing the plugin does not touch fonts you already installed with it, or the
system font state. If you want a clean slate as well, run `om-custom-font
reset-all` before removing (the overlay's *Reset to Omarchy defaults* button
does the same thing).
...
The plugin overlay additionally uses Qt6 declarative (`Qt.labs.folderlistmodel`
for the inline directory browser, and `FontLoader` for the pangram preview),
which the Omarchy shell already pulls in.

## Testing

```bash
make lint      # bash -n, JSON check on manifest, qmllint on every .qml
```

For the overlay itself, `bash -n` + `qmllint` is the offline story; the visual
pass is a real install:

```bash
omarchy plugin add file://$PWD --enable
omarchy-shell shell toggle io.github.whiskeytuesday.custom-font
```

A failed install is caught by the overlay's error banner rather than a broken
shell — no scratch profile needed.
